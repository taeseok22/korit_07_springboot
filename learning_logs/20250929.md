...
- 요청 처리의 단계
  ...
  3. Controller 실행 :
    ...
    6. View 렌더링 및 응답(View Rendering & Response) :
      ...
      - 쉽게 설명하면 `GET user/1`이면 DB 상에서의 user_id가 1인 애의 데이터를 이용하여 HTML 문서를 만들고 2라면 user_id가 2인 애의 데이터를 활용해 HTML 문서를 만들어서 브라우저로 보여준다 보면 됨.
...
1. 관심사 분리(Seperation of Concerns) :
  - 데이터 처리(Model), 화면 표시(View), 요청 처리(Controller)가 명확하게 분리되있어 각 부분을 독립적으로 개발하고 수정할 수 있음.
    - 보여주는 부분이 잘못됐다 -> View 수정
    - DB에서 가져오질 못한다 -> Repository or Service 수정
    - 요청 엔드포인트가 잘못됐다 -> Controller 수정 등
2. 높은 재사용성 :
  동일한 Model(비지니스 로직)을 사용해 여러 View(웹 페이지, 모바일 앱, REST API 등)를 만들 수 있음(대충 사이즈 줄여서 모바일 버전 웹 페이지를 만든다던지, 회원 가입 창을 재활용해 로그인 창으로 쓴다든지 등)
3. 유지 보수의 용이성 :
  - 화면 디자인이 변경되도 Controller나 Model 코드는 거의 수정할 필요가 없다. 반대로 비지니스 로직이 바껴도 View에 미치는 영향이 최소화됨.
  - 지금 카톡 생각해보면 View바꼇다고 볼 수 있다. 하지만 DB에 우리 프로필 사진들은 다 있다. 가장 최근 id를 기준으로 프로필 사진에 나머지 사진들은 커버 사진에 집어넣어놨던 걸 인스타식으로 고쳐놨다 보면 됨. 즉 DB에 있는 다른 부분들의 코드 수정이 막 엄청나게 일어나진 않았을 거란 말임.
4. 테스트 용이성 :
  - 각 구성 요소가 독립적이기 때문에 단위 테스트를 쉽게 작성하고 수행 가능. (GET 요청만 확인한다든지 등으로 금요일에 해봤음.)특히 Controller와 Model의 비지니스 로직은 View와 독립적으로 테스트 가능(postman, HeidiSQL로 CRUD 테스트를 마무리 지었다. 화면은 아예 없는데도 함.)
5. 병렬 개발 가능 :
  - 프론트엔드 개발자(View)와 백엔드 개발자(Controller, Model)가 서로의 작업에 크게 영향을 주지 않으면서 동시 개발이 가능해 속도가 향상됨.

## 요약
1. MVC는 Model /View / Controller로 _코드를 분리_ 하는 _디자인 패턴_ 이다.
2. `DispatcherServlet` 이라는 프론트 컨트롤러를 통해 요청 처리함.
3. Controller는 요청을 받고 Model을 호출해 View를 선택하는 중재자 역할을 함.
  - View에서 요청 (Request) -> Controller 내부의 코드에서 Service 호출 -> Service가 Repository를 호출해 DB관련 정보 습득 후 데이터 처리 -> Controller로 보내주면 -> Controller가 적절한 .html 문서를 응답(Response). / 우린 ResponseEntity 쓸거임.
4. Model은 비지니스 로직과 데이터를 담당(Entity - Repository - Service)

# Spring Initializr를 통해 cardatabase_4 만들거
http://start.spring.io

# 프로젝트 만든 후에 실행 여부 따졌을 때 실행이 되는 시점을 기준으로 함.
1. Car / CarRepository를 만들었다.
  - 근데 CarRepository 부분에서 JpaRepository를 상속 받으려 하니 검색이 안됐다.
    `implementation 'org.springframework.boot:spring-boot-starter-data-jpa'` 가 없어서.
  - 그래서 추가해주고 코끼리 눌렀음.
  - 여전히 오류 발생함. -> Car의 field로 Owner가 있는데 현재 Owner Entity가 없어서.
2. Owner / OwnerRepository를 생성.
  - 그리고 실행시키려 해보니 실행 안됨.
  - application.properties에 관련 설정들 아무것도 안넣어서.
  - build.gradle에 mariadb 관련 의존성 추가 안해서.
  - 이상의 설정들을 마무리 짓고 스프링부트 애플리케이션을 실행시켰을 때 결과값은?
    - HeidiSQL 켜서 `SELECT * FROM car;` 했을 때 row=0 인 테이블이 생성 완료 되있어야 함.
  - 하지만 Controller 파트를 작성하지 않았기 때문에 HeidiSQL에선 데이터를 확인 및 추가가 가능하지만 postman을 통해선 불가능하다는 것도 확인됐음.
3. 이상의 과정에서 생긴 문제를 보아 MVC 패턴을 만들어야 postman에서 CRUD를 할 수 있을 것 같다. 그럼 Model 파트에서 만든 Entity와 Repository를 제외하고 Service 파트도 만들어야하고 Controller 파트에 해당하는 Controller 클래스도 만들어야함.
  - 루트 프로젝트 패키지에서 service 패키지 생성 -> CarService 생성.

4. 페이지 보여줄 수 있도록 해야 하는데 이걸 React와 연결할거니까 View와 연결 하는건 아니고 ResponseEntity와 연결하기 위한 CarController 작성할거임
